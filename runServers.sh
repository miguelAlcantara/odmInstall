...
function usage(){
	echo "Usage: sh runServers.sh -r params / -u params / -d params"
	echo "r - restart ( Restarts all the servers indicated as parameters )"
	echo "u - up OR start ( Starts all the servers indicated as parameters )"
	echo "d - down OR stop ( Starts all the servers indicated as parameters )"
	echo ""
	echo "Parameters:"
	echo "all - DMGR / Nodeagent / Decision Center / Decision Server "
	echo "dCenter - Decision Center "
	echo "dServer - Decision Server "
	echo "nodeagent - Nodeagent"
	echo "dmgr - DMGR "
	echo "application - Decision Center / Decision Server "
}

while getopts ":r::d::u::" opt; do
  case $opt in
    r)
		case $OPTARG in
			all)
